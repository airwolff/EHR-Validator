...
def test_agent_findings_never_land_in_validation_issues(tmp_path, monkeypatch):
...
def test_recording_a_result_takes_the_record_out_of_the_inbox(tmp_path, monkeypatch):
    store = _fresh_store(tmp_path, monkeypatch)
...
def test_cleared_record_is_not_reoffered(tmp_path, monkeypatch):
...
def test_findings_and_marker_commit_together(tmp_path, monkeypatch):
...
def test_unknown_run_id_is_loud_not_silent(tmp_path, monkeypatch):
    store = _fresh_store(tmp_path, monkeypatch)
    with pytest.raises(ValueError, match="does not exist"):
        store.record_agent_result(9999, [_finding()], batch_date="2026-07-13")


def test_worklist_is_scoped_to_its_batch_date(tmp_path, monkeypatch):
    store = _fresh_store(tmp_path, monkeypatch)
...
def test_worklist_ties_hold_insertion_order(tmp_path, monkeypatch):
...
def test_admin_domain_is_ranked_not_silently_last(tmp_path, monkeypatch):
...
def test_non_dict_payload_does_not_kill_the_batch(tmp_path, monkeypatch):
